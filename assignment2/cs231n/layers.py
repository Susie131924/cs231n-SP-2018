...
def affine_forward(x, w, b):
    ...
    - cache: (x, w, b)
    """
    x_2 = x.reshape(x.shape[0], int(x.size/x.shape[0]))
    out = x_2.dot(w) + b
    ###########################################################################
...
def affine_backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], int(x.size/x.shape[0])).T.dot(dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = gamma * x_norm + beta
        cache = (x, gamma, beta, x_norm, running_mean, running_var, eps)
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    dx_mean1 = 1 / np.sqrt(var + eps) * dx_norm #N,D
    #print(dx1.shape)
    
    d1 = np.sum(dx_norm * (x - mean),axis = 0)
    d2 = d1 * -1 * np.sqrt(var + eps) ** -2
    d3 = d2 * 0.5 * (var + eps) ** -0.5
    dvar = d3
    #dvar = np.sum(dx_norm * (x - mean) * (-0.5), axis = 0) * ((var + eps) ** -2.5) #D
    #print(dvar.shape)
    
    dx_mean2 = dvar * (1/N) * np.ones_like(x) * 2 * (x - mean)#N,D
    #print(dx3.shape)
    
    dx_mean = dx_mean1 + dx_mean2 #N,D
    #print(dx_mean.shape)
    
    dx1 = dx_mean #N,D
    
    dmean = - np.sum(dx_mean, axis = 0) #D
    
    dx2 = (1/N) * dmean * np.ones_like(x) #N,D
    #print(dx2.shape)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    #for i in range(D):
        #x_norm[:,i] = (x[:,i] - sample_mean) / np.sqrt(sample_var + eps)
    x_norm = (x - (sample_mean)[:,None]) / np.sqrt((sample_var)[:,None] + eps)
    #print(gamma.shape,x_norm.shape)
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(h_out):
        for j in range(w_out):
            ...
            for f in range(F):
                dw[f,:,:,:] += np.sum((dout[:,f,i,j])[:,None,None,None] * mask,axis = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(h_out):
        for j in range(w_out):
            out[:,:,i,j] = np.max(x[:,:,i*stride:i*stride+p_h, j*stride:j*stride+p_w], axis=(2,3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(h_out):
        for j in range(w_out):
            cur_x = x[:,:,i*stride:i*stride+p_h, j*stride:j*stride+p_w]
            out = np.max(cur_x, axis=(2,3))
            mask = (cur_x == (out)[:,:,None,None])
            dx[:,:,i*stride:i*stride+p_h, j*stride:j*stride+p_w] += mask * (dout[:,:,i,j])[:,:,None,None]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    mean = np.mean(x,axis = (2,3,4))
    var = np.var(x,axis = (2,3,4))
